...
const postPostLikeCount = async (results, postId) => {
  try {
    await connection;
    try {
      const post = await Post.findByIdAndUpdate(postId, { $inc: { postLikes: 1 } }, { new: true });
      if (post) {
        results.post = post;
      }
      else {
      ...
      results.result = false;
      results.error.push("Repository Error - postPostLikeCound");
    }
  } catch (err) {
    results.result = false;
    results.error.push("DB connection Error");
...
const getPostsById = async (results, postIds) => {
  try {
    await connection;
    try {
      ...
      if (posts.length > 0) {
        ...
        results.error.push("No posts found for the given IDs");
      }
    } catch (err) {
      results.result = false;
      results.error.push("Repository Error");
...
const deleteDecrementPostLikeCount = async (results, postId) => {
  try {
    await connection;
    try {
      const post = await Post.findByIdAndUpdate(postId, { $inc: { postLikes: -1 } }, { new: true });
      if (post) {
        ...
        results.error.push('Post not found');
      }
    } catch (err) {
      results.result = false;
      results.error.push('Repository Error - decrementPostLikeCount');
    }
  } catch (err) {
    results.result = false;
    results.error.push('DB connection Error');
...
export { getAllPosts, getPostByLocationId, postPostByLocationId, getPostByPostId, deletePostByPostId, getPostByUserId, postPostLikeCount, getPostsById, deleteDecrementPostLikeCount };
