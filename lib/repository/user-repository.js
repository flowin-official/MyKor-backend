...
const postUser = async (results, userData) => {
  try {
    await connection;
    try {
      const existUser = await User.findOne({ kakaoId: userData.kakaoUserCode });
      if (existUser) {
        results.result = false;
        results.error.push('Existing User Error');
        return;
      }
      const newUser = new User({ kakaoId: userData.kakaoUserCode, userName: userData.userName, userEmail: userData.userEmail });
      try {
...
export { getAllUsers, postUser, getUserById, postUserLikePost, getUserLikePost, deleteUserLikePost };